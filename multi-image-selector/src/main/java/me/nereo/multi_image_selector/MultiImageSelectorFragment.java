    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        switch (filterType) {
            case MultiFilterType.IMAGE:
                categoryText = getString(R.string.mis_folder_image_all);
                break;
            case MultiFilterType.VIDEO:
                categoryText = getString(R.string.mis_folder_video_all);
                break;
            default:
                categoryText = getString(R.string.mis_folder_all);
                break;
        }
...
//                }
        ...
        mFolderAdapter.setFilterType(filterType);
    }
                        ...
                        if (folder == null) {
                            ...
                            if (index == 0) {
                                switch (filterType) {
                                    case MultiFilterType.IMAGE:
                                        categoryText = getString(R.string.mis_folder_image_all);
                                        filterCacheType = MultiFilterType.IMAGE;
                                        break;
                                    case MultiFilterType.VIDEO:
                                        categoryText = getString(R.string.mis_folder_video_all);
                                        filterCacheType = MultiFilterType.VIDEO;
                                        break;
                                    default:
                                        categoryText = getString(R.string.mis_folder_all);
                                        filterCacheType = MultiFilterType.ALL;
                                        break;
                                }
                            } else if (index == 1) {
                                categoryText = getString(R.string.mis_folder_image_all);
...
//                        mCallback.onCameraShot(mTmpFile);
                        ...
                        mImageAdapter.getItemsData().add(new Image(mTmpFile.getPath(), mTmpFile.getName(), mTmpFile.lastModified(), FileUtils.getMimeType(mTmpFile)));
    ...
    private Uri getTempFile() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            return FileProvider.getUriForFile(getActivity(), getActivity().getPackageName() + ".provider", mTmpFile);
        } else {
    ...
    private LoaderManager.LoaderCallbacks<Cursor> mLoaderCallback = new LoaderManager.LoaderCallbacks<Cursor>() {
        ...
        public Loader<Cursor> onCreateLoader(int id, Bundle args) {
            ...
            if (filterCacheType == MultiFilterType.IMAGE) {
                if (id == LOADER_ALL) {
            ...
            } else if (filterCacheType == MultiFilterType.VIDEO) {
                if (id == LOADER_ALL) {
                    cursorLoader = new CursorLoader(getActivity()
                            , MediaStore.Video.Media.EXTERNAL_CONTENT_URI
                            , VIDEO_PROJECTION
                            , VIDEO_PROJECTION[4] + ">0 AND "
                            + VIDEO_PROJECTION[3] + "=? OR "
                            + VIDEO_PROJECTION[3] + "=? OR "
                            + VIDEO_PROJECTION[3] + "=? OR "
                            + VIDEO_PROJECTION[3] + "=? OR "
                            + VIDEO_PROJECTION[3] + "=? OR "
                            + VIDEO_PROJECTION[3] + "=? "
                            , MultiFilterType.VIDEO_TYPE
                    ...
                    cursorLoader = new CursorLoader(getActivity()
                            ...
                            , VIDEO_PROJECTION
                            , VIDEO_PROJECTION[4] + ">0 AND "
                            + VIDEO_PROJECTION[0] + " like '%" + args.getString("path") + "%'"
                            , null
                            , VIDEO_PROJECTION[2] + " DESC");
                }
            } else if (filterCacheType == MultiFilterType.ALL) {
                if (id == LOADER_ALL) {
                    cursorLoader = new CursorLoader(getActivity()
                            , MediaStore.Files.getContentUri("external")
                            , All_PROJECTION
                            , All_PROJECTION[4] + ">0 AND "
                            + All_PROJECTION[3] + "=? OR "
                            + All_PROJECTION[3] + "=? OR "
                            + All_PROJECTION[3] + "=? OR "
                            + All_PROJECTION[3] + "=? OR "
                            ...
                            + All_PROJECTION[3] + "=? OR "
                            + All_PROJECTION[3] + "=? "
                            , MultiFilterType.ALL_TYPE
                            , All_PROJECTION[2] + " DESC");
                } else if (id == LOADER_CATEGORY) {
                    cursorLoader = new CursorLoader(getActivity()
                            , MediaStore.Files.getContentUri("external")
                            , All_PROJECTION
                            , All_PROJECTION[4] + ">0 AND "
                            + All_PROJECTION[0] + " like '%" + args.getString("path") + "%'"
                            , null
                            , All_PROJECTION[2] + " DESC");
                }
            } else {
                ...
                if (data.getCount() > 0) {
                    ...
                    do {
                        ...
                        if (filterCacheType == MultiFilterType.IMAGE) {
                            ...
                            mimeType = data.getString(data.getColumnIndexOrThrow(IMAGE_PROJECTION[3]));
                        } else if (filterCacheType == MultiFilterType.VIDEO) {
                            ...
                            mimeType = data.getString(data.getColumnIndexOrThrow(VIDEO_PROJECTION[3]));
                        } else {
                            path = data.getString(data.getColumnIndexOrThrow(All_PROJECTION[0]));
                            name = data.getString(data.getColumnIndexOrThrow(All_PROJECTION[1]));
                            dateTime = data.getLong(data.getColumnIndexOrThrow(All_PROJECTION[2]));
                            mimeType = data.getString(data.getColumnIndexOrThrow(All_PROJECTION[3]));
                        }
                        if (!fileExist(path)) {
                            continue;
                        }
                        ...
                        if (!TextUtils.isEmpty(name)) {
                            image = new Image(path, name, dateTime, mimeType);
                            images.add(image);
                            if (!hasFolderGened) {
                                // get all folder data
    ...
    private int filterType() {
        return getArguments() == null ? MultiFilterType.IMAGE : getArguments().getInt(EXTRA_FILTER_TYPE, MultiFilterType.IMAGE);
    }

    private boolean showCamera() {
        return filterType() == MultiFilterType.IMAGE && (getArguments() == null || getArguments().getBoolean(EXTRA_SHOW_CAMERA, true));
    }
