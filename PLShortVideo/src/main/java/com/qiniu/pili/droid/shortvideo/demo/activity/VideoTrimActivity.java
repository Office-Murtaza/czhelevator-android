...
public class VideoTrimActivity extends Activity {
    ...
    public static final String VIDEO_TRIM_DURATION = "VideoTrimDuration";
...
//        startActivityForResult(Intent.createChooser(intent, "选择要导入的视频"), 0);
        videoPath = getIntent().getStringExtra(VIDEO_FILE_PATH);
        videoMaxDuration = getIntent().getLongExtra(VIDEO_TRIM_DURATION, 15 * 1000);
        init(videoPath);

        showDurationTip(true, videoMaxDuration);
    }
...
//        mShortVideoTrimmer = new PLShortVideoTrimmer(this, videoPath, Config.TRIM_FILE_PATH);
        mShortVideoTrimmer = new PLShortVideoTrimmer(this, videoPath, new File(Config.VIDEO_STORAGE_DIR, String.format("trimmed_%s.mp4", System.currentTimeMillis())).getAbsolutePath());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == VideoRecordActivity.RECORD_EDIT_REQUEST) {
                if (data != null) {
                    Intent intent = new Intent();
                    intent.putExtra(VideoRecordActivity.RECORD_RESULT, data.getStringExtra(VideoRecordActivity.RECORD_RESULT));
    ...
    public void onDone(View v) {
        if ((mSelectedEndMs - mSelectedBeginMs) > videoMaxDuration) {
            showDurationTip(false, videoMaxDuration);
            return;
        }
...
//                VideoEditActivity.start(VideoTrimActivity.this, path);
                VideoEditActivity.startResult(VideoTrimActivity.this, path, VideoRecordActivity.RECORD_EDIT_REQUEST);
            }
    ...
    private void showDurationTip(boolean first, long duration) {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle(getString(R.string.tip_trim_video_duration, duration / 1000));
        builder.setPositiveButton(getString(first ? R.string.dlg_start : R.string.dlg_continue), null);
        builder.create().show();
    }
